...
function criterionText(test: { studyCriterion: { kind: string; currentVersion: { minValue: number | null; maxValue: number | null; expectedText: string | null; expectedBoolean: boolean | null } | null } | null }) {
  const criterion = test.studyCriterion;
  const v = criterion?.currentVersion;
  if (!criterion || !v) return "Informacyjnie";
...
export default async function LabSamplePage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ saved?: string; conflicts?: string }> }) {
  ...
  const sample = await prisma.sample.findUnique({
    ...
    include: {
      study: {
        include: {
          client: true,
          samples: { select: { id: true, initialMeasurement: true } },
        },
      },
  ...
  return (
    <AppShell user={user} active="lab">
      <div className="topline">
        <div>
          <div className="eyebrow">{sample.study.studyNumber} · {sample.study.client.name}</div>
          <h1>{sample.code}</h1>
          <div className="subtle">{sample.study.projectName} · {sample.storageCondition} · {sample.checkpointLabel || "RF"}</div>
        </div>
        <div className="top-actions"><Link className="btn btn-secondary" href="/lab">← Laboratorium</Link><Link className="btn btn-secondary" href={`/studies/${sample.studyId}`}>Karta badania</Link></div>
      </div>

      {feedback.saved && <div className="save-banner success-banner">Zapisano {feedback.saved} {feedback.saved === "1" ? "wynik" : "wyników"}.</div>}
      {feedback.conflicts && <div className="save-banner danger-banner">Nie nadpisano {feedback.conflicts} wyników zmienionych równolegle przez innego użytkownika. Odświeżone wartości są poniżej.</div>}

      <section className="sample-hero">
        <div><span>Rola</span><strong>{sample.role === SampleRole.REFERENCE ? "RF / OOS backup" : sample.role === SampleRole.MICROBIOLOGY ? "Mikrobiologia" : "Próbka standardowa"}</strong></div>
        <div><span>Termin nominalny</span><strong>{sample.nominalDate?.toLocaleDateString("pl-PL") ?? "Brak — RF"}</strong></div>
        <div><span>Postęp</span><strong>{sample.role === SampleRole.MICROBIOLOGY ? (sample.microbiologyResult ? "1 / 1" : "0 / 1") : `${completedTests} / ${sample.tests.length}`}</strong></div>
        <div><span>NOK</span><strong className={nokCount ? "danger-text" : ""}>{nokCount}</strong></div>
      </section>
      ...
      <section className="section">
        ...
        <form action={saveInitialMeasurementAction} className="measurement-card">
      ...
      {allInitialDone && sample.study.status === StudyStatus.ACTIVE && sample.role !== SampleRole.MICROBIOLOGY && !(sample.role === SampleRole.REFERENCE && sample.referenceStatus === ReferenceStatus.AVAILABLE) && (
        <section className="section">
          ...
          <form action={saveSampleResultsAction} className="results-form">
            <input type="hidden" name="sampleId" value={sample.id} />
            <div className="results-table-head"><span>Badanie</span><span>Kryterium</span><span>Wynik</span><span>Ocena</span><span>Stan</span><span>Powód / historia</span></div>
            {sample.tests.map((test) => {
              const result = test.result;
              const values = valuesFor(test.testDefinition.code);
              const initial = test.testDefinition.code === "WEIGHT" ? sample.initialMeasurement?.initialWeightG : test.testDefinition.code === "PRESSURE" ? sample.initialMeasurement?.initialPressureBar : null;
              const actual = result?.numericValue ?? null;
              const difference = initial != null && actual != null ? actual - initial : null;
              const percent = initial != null && actual != null && initial !== 0 ? ((actual - initial) / initial) * 100 : null;
              return <div className="result-row" key={test.id}>
                <div><strong>{test.testDefinition.name}</strong><small>{test.testDefinition.unit || test.testDefinition.category}{initial != null && <><br />Bazowa: {initial}{test.testDefinition.unit ? ` ${test.testDefinition.unit}` : ""}</>}</small></div>
                <div className="criterion-value">{criterionText(test)}</div>
                <div>
                  {test.testDefinition.valueType === TestValueType.NUMBER ? <input name={`value_${test.id}`} type="number" step="any" defaultValue={result?.numericValue ?? ""} disabled={!canEdit} /> : test.testDefinition.valueType === TestValueType.BOOLEAN ? <select name={`value_${test.id}`} defaultValue={result?.booleanValue == null ? "" : result.booleanValue ? "true" : "false"} disabled={!canEdit}><option value="">—</option><option value="true">TAK</option><option value="false">NIE</option></select> : values.length ? <select name={`value_${test.id}`} defaultValue={result?.textValue ?? ""} disabled={!canEdit}><option value="">Wybierz</option>{values.map((value) => <option key={value} value={value}>{value}</option>)}</select> : <input name={`value_${test.id}`} defaultValue={result?.textValue ?? ""} disabled={!canEdit} />}
                  {difference != null && <small className="delta-note">Δ {difference >= 0 ? "+" : ""}{difference.toFixed(2)} · {percent == null ? "—" : `${percent >= 0 ? "+" : ""}${percent.toFixed(2)}%`}</small>}
                </div>
                ...
                <div><select name={`state_${test.id}`} defaultValue={result?.state ?? ResultState.RECORDED} disabled={!canEdit}><option value={ResultState.RECORDED}>Wynik</option><option value={ResultState.NOT_PERFORMED}>Nie wykonano</option></select><input type="hidden" name={`version_${test.id}`} value={result?.version ?? ""} /></div>
                <div><input name={`reason_${test.id}`} placeholder="Powód, jeśli nie wykonano" defaultValue={result?.notPerformedReason ?? ""} disabled={!canEdit} />{result && <small>{result.author.name} · {result.updatedAt.toLocaleString("pl-PL")}</small>}</div>
              </div>;
            })}
