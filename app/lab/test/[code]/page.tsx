...
import { saveBatchResultsAction } from "@/app/lab/batch-actions";
...
export default async function BatchTestPage({ params, searchParams }: { params: Promise<{ code: string }>; searchParams: Promise<{ saved?: string; conflicts?: string; errors?: string }> }) {
  ...
  const rowsRaw = await prisma.sampleTest.findMany({
    where: {
      ...
      sample: { study: { status: StudyStatus.ACTIVE } },
    },
      ...
      sample: {
        include: {
          ...
          study: {
            include: {
              client: true,
              samples: { select: { initialMeasurement: true } },
            },
          },
        },
      },
  ...
  return (
    <AppShell user={user} active="lab">
      <div className="topline">
        <div><div className="eyebrow">Laboratorium · tryb seryjny</div><h1>{testDefinition.name}</h1><div className="subtle">Wprowadzaj wyniki wielu próbek bez otwierania każdej z osobna.</div></div>
        <div className="top-actions"><Link className="btn btn-secondary" href="/lab?mode=test">← Lista badań</Link></div>
      </div>

      {(feedback.saved || feedback.conflicts || feedback.errors) && <div className="batch-feedback">
        <span className="badge success">Zapisano: {feedback.saved ?? 0}</span>
        {Number(feedback.conflicts ?? 0) > 0 && <span className="badge warning">Konflikty: {feedback.conflicts}</span>}
        {Number(feedback.errors ?? 0) > 0 && <span className="badge danger">Błędy walidacji: {feedback.errors}</span>}
      </div>}
      ...
      <section className="section">
        ...
        {rows.length === 0 ? <div className="empty-state large-empty"><strong>Brak próbek dla tego badania</strong><span>Nie ma teraz wyników do wprowadzenia w tym trybie.</span></div> : (
          <form action={saveBatchResultsAction} className="batch-form">
            <input type="hidden" name="testCode" value={code} />
            <div className="batch-table-card">
              <div className="batch-table-head"><span>Status</span><span>Próbka / projekt</span><span>Termin / warunek</span><span>Kryterium</span><span>Wynik</span><span>Ocena</span><span>Stan / powód</span></div>
              {rows.map((row) => {
                const t = timing(row.sample);
                const result = row.result;
                const initial = code === "WEIGHT" ? row.sample.initialMeasurement?.initialWeightG : code === "PRESSURE" ? row.sample.initialMeasurement?.initialPressureBar : null;
                return <div className="batch-row" key={row.id}>
                  <input type="hidden" name="sampleTestId" value={row.id} />
                  <input type="hidden" name={`version_${row.id}`} value={result?.version ?? ""} />
