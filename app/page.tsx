...
export default async function Home() {
  ...
  if (user.role === UserRole.LAB_TECHNICIAN) redirect("/lab");
  ...
  const studies = await prisma.study.findMany({
    ...
    include: {
      ...
      samples: { include: { tests: { include: { result: true } }, microbiologyResult: true } },
    },
  });

  const completedThisYear = await prisma.study.count({
    where: { status: StudyStatus.COMPLETED, completedAt: { gte: new Date(new Date().getFullYear(), 0, 1) } },
  });

  const rows = studies.map((study) => {
    const executableSamples = study.samples.filter((sample) => sample.role !== SampleRole.REFERENCE || sample.referenceStatus === "ACTIVATED");
    ...
    return { study, completed, total: executableSamples.length, nok, late, overdue, nextDate: futureDates[0] ?? null };
  });
  ...
  return (
    <AppShell user={user} active="dashboard">
      ...
      <section className="section" id="attention">
        <div className="section-head"><div><div className="eyebrow">Priorytet</div><div className="section-title">Wymaga uwagi</div></div></div>
        <div className="table-card">
          {attentionRows.length === 0 ? <div className="empty-state"><strong>Brak elementów wymagających uwagi</strong><span>Nie ma aktywnych NOK ani zaległości.</span></div> : <table>
            <thead><tr><th>Badanie</th><th>Klient</th><th>NOK</th><th>Zaległe</th><th>Przeterm.</th><th>Postęp</th></tr></thead>
            <tbody>{attentionRows.map((row) => <tr key={row.study.id}>
      ...
      <section className="section">
        <div className="section-head"><div><div className="eyebrow">Portfel badań</div><div className="section-title">Wszystkie aktywne badania</div></div><Link href="/studies" className="section-link">Zobacz wszystkie →</Link></div>
        <div className="table-card"><table>
          ...
          <tbody>{rows.map((row) => {
            ...
            return <tr key={row.study.id}>
              ...
              <td>{row.study.client.name}</td><td>{row.study.responsibleTechnologist.name}</td>
